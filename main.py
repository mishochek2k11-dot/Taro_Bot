...
# Хранилище в памяти (сбросится при перезапуске, но для теста пойдёт)
users = {}
    ...
    def do_POST(self):
        ...
        try:
            ...
            if "message" in update:
                ...
                if is_admin(user_id):
                    send_message(chat_id, "✨ Привет, создатель! Режим админа — безлимитные расклады.")
                    keyboard = [
                ...
                if data_cb == "status":
                    ...
                    if not can_use():
                        edit_message(chat_id, message_id, f"❌ Лимит {FREE_ATTEMPTS} раскладов в месяц исчерпан.")
                        return
